...
class FritzingWemosFetcher {
  ...
  async fetchFileList() {
    try {
      ...
      const wemosFzpFiles = fzpFiles.filter(file => 
        file.name.toLowerCase().includes('wemos') && file.name.endsWith('.fzp')
      );

      // Fetch SVG files
      const svgFiles = await this.fetchFilesFromUrl(this.svgApiUrl);
      const wemosSvgFiles = svgFiles.filter(file => 
        file.name.toLowerCase().includes('wemos') && file.name.endsWith('.svg')
      );

      console.log(`Found ${wemosFzpFiles.length} Wemos FZP files:`);
      wemosFzpFiles.forEach((file, index) => {
        console.log(`${index + 1}. ${file.name}`);
      });

      console.log(`\nFound ${wemosSvgFiles.length} Wemos SVG files:`);
      wemosSvgFiles.forEach((file, index) => {
        console.log(`${index + 1}. ${file.name}`);
      });

      return {
        fzpFiles: wemosFzpFiles,
        svgFiles: wemosSvgFiles
      };
    } catch (error) {
  ...
  async fetchFilesFromUrl(url) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'MicroPinout-Wemos-Fetcher/1.0',
  ...
  // Group files by Wemos board (fzp + associated svg files)
  groupWemosFiles(fzpFiles, svgFiles) {
    const groups = {};
  ...
  extractBaseName(fileName) {
    ...
    // Normalize common variations for Wemos
    baseName = baseName
      .replace(/\(fix\)/g, '')
      .replace(/\(1\)/g, '')
      .replace(/-bottom$/g, '')
      ...
      .replace(/-d1-pro$/g, '')
      .replace(/-d1-r32$/g, '')
      .replace(/-d1-mini-pro$/g, '')
      .replace(/-lolin32$/g, '')
      .replace(/-lolin32-lite$/g, '')
      .replace(/-lolin32-pro$/g, '')
      .replace(/-breakout$/g, '')
      .replace(/-shield$/g, '')
      .replace(/-sensor$/g, '')
      .replace(/-module$/g, '')
  ...
  async uploadToSystem(processedData, fileName) {
    try {
      const boardData = {
        ...
        manufacturer: processedData.boardMetadata.manufacturer || 'Wemos',
  ...
  async run() {
    try {
      console.log('🚀 Starting Fritzing Wemos Fetcher...\n');
      
      this.ensureTempDir();
      
      // Step 1: Fetch file lists
      console.log('📋 Fetching Wemos files from Fritzing repository...');
      const fileLists = await this.fetchFileList();
      
      if (fileLists.fzpFiles.length === 0) {
        console.log('❌ No Wemos FZP files found in the repository');
        return;
      }

      // Step 2: Group files by Wemos board
      console.log('\n📦 Grouping files by Wemos board...');
      const groupedFiles = this.groupWemosFiles(fileLists.fzpFiles, fileLists.svgFiles);
      const groupNames = Object.keys(groupedFiles);
      
      console.log(`Found ${groupNames.length} complete Wemos boards:`);
      groupNames.forEach((name, index) => {
        console.log(`${index + 1}. ${name} (${groupedFiles[name].svgs.length} SVG files)`);
      });

      if (groupNames.length === 0) {
        console.log('❌ No complete Wemos boards found (missing FZP or SVG files)');
        return;
      }

      console.log(`\n📥 Processing ${groupNames.length} Wemos boards...\n`);
      
      // Step 3: Process each Wemos board
      ...
      // Step 4: Summary
      console.log('\n🎉 Processing Complete!');
      console.log(`✅ Successfully processed: ${this.uploadedBoards.length}/${boardsToProcess.length} Wemos boards`);
      
      if (this.uploadedBoards.length > 0) {
        console.log('\n📊 Summary of processed Wemos boards:');
...
if (require.main === module) {
  const fetcher = new FritzingWemosFetcher();
  fetcher.run().catch(console.error);
}

module.exports = FritzingWemosFetcher;
