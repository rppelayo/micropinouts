...
class FritzingSparkFunFetcher {
  ...
  async fetchFileList() {
    try {
      ...
      const sparkfunFzpFiles = fzpFiles.filter(file => 
        file.name.toLowerCase().includes('sparkfun') && file.name.endsWith('.fzp')
      );

      // Fetch SVG files
      const svgFiles = await this.fetchFilesFromUrl(this.svgApiUrl);
      const sparkfunSvgFiles = svgFiles.filter(file => 
        file.name.toLowerCase().includes('sparkfun') && file.name.endsWith('.svg')
      );

      console.log(`Found ${sparkfunFzpFiles.length} SparkFun FZP files:`);
      sparkfunFzpFiles.forEach((file, index) => {
        console.log(`${index + 1}. ${file.name}`);
      });

      console.log(`\nFound ${sparkfunSvgFiles.length} SparkFun SVG files:`);
      sparkfunSvgFiles.forEach((file, index) => {
        console.log(`${index + 1}. ${file.name}`);
      });

      return {
        fzpFiles: sparkfunFzpFiles,
        svgFiles: sparkfunSvgFiles
      };
    } catch (error) {
  ...
  async fetchFilesFromUrl(url) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'MicroPinout-SparkFun-Fetcher/1.0',
  ...
  // Group files by SparkFun board (fzp + associated svg files)
  groupSparkFunFiles(fzpFiles, svgFiles) {
    const groups = {};
  ...
  extractBaseName(fileName) {
    ...
    // Normalize common variations for SparkFun
    baseName = baseName
      .replace(/\(fix\)/g, '')
      .replace(/\(1\)/g, '')
      .replace(/-bottom$/g, '')
  ...
  async uploadToSystem(processedData, fileName) {
    try {
      const boardData = {
        ...
        manufacturer: processedData.boardMetadata.manufacturer || 'SparkFun Electronics',
  ...
  async run() {
    try {
      console.log('🚀 Starting Fritzing SparkFun Fetcher...\n');
      
      this.ensureTempDir();
      
      // Step 1: Fetch file lists
      console.log('📋 Fetching SparkFun files from Fritzing repository...');
      const fileLists = await this.fetchFileList();
      
      if (fileLists.fzpFiles.length === 0) {
        console.log('❌ No SparkFun FZP files found in the repository');
        return;
      }

      // Step 2: Group files by SparkFun board
      console.log('\n📦 Grouping files by SparkFun board...');
      const groupedFiles = this.groupSparkFunFiles(fileLists.fzpFiles, fileLists.svgFiles);
      const groupNames = Object.keys(groupedFiles);
      
      console.log(`Found ${groupNames.length} complete SparkFun boards:`);
      groupNames.forEach((name, index) => {
        console.log(`${index + 1}. ${name} (${groupedFiles[name].svgs.length} SVG files)`);
      });

      if (groupNames.length === 0) {
        console.log('❌ No complete SparkFun boards found (missing FZP or SVG files)');
        return;
      }

      console.log(`\n📥 Processing ${groupNames.length} SparkFun boards...\n`);
      
      // Step 3: Process each SparkFun board
      ...
      // Step 4: Summary
      console.log('\n🎉 Processing Complete!');
      console.log(`✅ Successfully processed: ${this.uploadedBoards.length}/${boardsToProcess.length} SparkFun boards`);
      
      if (this.uploadedBoards.length > 0) {
        console.log('\n📊 Summary of processed SparkFun boards:');
...
if (require.main === module) {
  const fetcher = new FritzingSparkFunFetcher();
  fetcher.run().catch(console.error);
}

module.exports = FritzingSparkFunFetcher;
