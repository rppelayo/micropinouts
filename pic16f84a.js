...
// Systematic pinout generation
function generateSystematicPinout() {
    const pinCount = Object.keys(pic16f84aData.pins).length; // 18 pins
    const chipDiagram = document.querySelector('.chip-diagram');
    
    if (!chipDiagram) return;
    
    // Set CSS variables for systematic calculation
    chipDiagram.style.setProperty('--pin-count', pinCount);
    chipDiagram.style.setProperty('--chip-height', '300px');
    chipDiagram.style.setProperty('--pin-spacing', `${300 / pinCount}px`);
    
    // Generate left pins (1-9)
    const leftPins = generatePinColumn(1, 9, 'left');
    
    // Generate right pins (10-18) 
    const rightPins = generatePinColumn(10, 18, 'right');
...
function generatePin(pinNum, side) {
    ...
    // Calculate pin position for systematic spacing
    let pinPosition;
    if (side === 'left') {
        pinPosition = pinNum - 1; // 0-8 for pins 1-9
    } else {
        pinPosition = pinNum - 10; // 0-8 for pins 10-18
    }
    pin.setAttribute('data-pin-position', pinPosition);
    ...
    if (pinData) {
        ...
        if (pinData.functions.includes('Power')) {
            if (pinData.name.includes('VDD')) {
                pinLabel.classList.add('power-positive');
            } else if (pinData.name.includes('VSS')) {
                pinLabel.classList.add('power-negative');
            }
        } else if (pinData.functions.includes('Digital I/O')) {
            pinLabel.classList.add('gpio');
        } else if (pinData.functions.includes('Clock')) {
            pinLabel.classList.add('clock');
        } else if (pinData.functions.includes('Reset')) {
...
function generateChipBody(pinCount) {
    ...
    for (let i = 1; i <= 9; i++) {
        ...
        pinNum.setAttribute('data-pin-position', i - 1);
        leftNumbers.appendChild(pinNum);
    }
    
    // Add right pin numbers (10-18)
    ...
    for (let i = 10; i <= 18; i++) {
        ...
        pinNum.className = 'pin-num';
        pinNum.textContent = i;
        pinNum.setAttribute('data-pin-position', i - 10);
