...
class FritzingRaspberryPiFetcher {
  ...
  async fetchFileList() {
    try {
      ...
      const raspberryPiFzpFiles = fzpFiles.filter(file => 
        file.name.toLowerCase().includes('raspberry') && file.name.endsWith('.fzp')
      );

      // Fetch SVG files
      const svgFiles = await this.fetchFilesFromUrl(this.svgApiUrl);
      const raspberryPiSvgFiles = svgFiles.filter(file => 
        file.name.toLowerCase().includes('raspberry') && file.name.endsWith('.svg')
      );

      console.log(`Found ${raspberryPiFzpFiles.length} Raspberry Pi FZP files:`);
      raspberryPiFzpFiles.forEach((file, index) => {
        console.log(`${index + 1}. ${file.name}`);
      });

      console.log(`\nFound ${raspberryPiSvgFiles.length} Raspberry Pi SVG files:`);
      raspberryPiSvgFiles.forEach((file, index) => {
        console.log(`${index + 1}. ${file.name}`);
      });

      return {
        fzpFiles: raspberryPiFzpFiles,
        svgFiles: raspberryPiSvgFiles
      };
    } catch (error) {
  ...
  async fetchFilesFromUrl(url) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: {
          'User-Agent': 'MicroPinout-RaspberryPi-Fetcher/1.0',
  ...
  // Group files by Raspberry Pi board (fzp + associated svg files)
  groupRaspberryPiFiles(fzpFiles, svgFiles) {
    const groups = {};
  ...
  extractBaseName(fileName) {
    ...
    // Normalize common variations for Raspberry Pi
    baseName = baseName
      .replace(/\(fix\)/g, '')
      .replace(/\(1\)/g, '')
      .replace(/-bottom$/g, '')
  ...
  async uploadToSystem(processedData, fileName) {
    try {
      const boardData = {
        ...
        manufacturer: processedData.boardMetadata.manufacturer || 'Raspberry Pi Foundation',
        package_type: processedData.boardMetadata.package_type || 'Unknown',
        voltage_range: processedData.boardMetadata.voltage_range || '3.3V/5V',
        clock_speed: processedData.boardMetadata.clock_speed || '1.5GHz',
  ...
  async run() {
    try {
      console.log('🚀 Starting Fritzing Raspberry Pi Fetcher...\n');
      
      this.ensureTempDir();
      
      // Step 1: Fetch file lists
      console.log('📋 Fetching Raspberry Pi files from Fritzing repository...');
      const fileLists = await this.fetchFileList();
      
      if (fileLists.fzpFiles.length === 0) {
        console.log('❌ No Raspberry Pi FZP files found in the repository');
        return;
      }

      // Step 2: Group files by Raspberry Pi board
      console.log('\n📦 Grouping files by Raspberry Pi board...');
      const groupedFiles = this.groupRaspberryPiFiles(fileLists.fzpFiles, fileLists.svgFiles);
      const groupNames = Object.keys(groupedFiles);
      
      console.log(`Found ${groupNames.length} complete Raspberry Pi boards:`);
      groupNames.forEach((name, index) => {
        console.log(`${index + 1}. ${name} (${groupedFiles[name].svgs.length} SVG files)`);
      });

      if (groupNames.length === 0) {
        console.log('❌ No complete Raspberry Pi boards found (missing FZP or SVG files)');
        return;
      }

      console.log(`\n📥 Processing ${groupNames.length} Raspberry Pi boards...\n`);
      
      // Step 3: Process each Raspberry Pi board
      ...
      // Step 4: Summary
      console.log('\n🎉 Processing Complete!');
      console.log(`✅ Successfully processed: ${this.uploadedBoards.length}/${boardsToProcess.length} Raspberry Pi boards`);
      
      if (this.uploadedBoards.length > 0) {
        console.log('\n📊 Summary of processed Raspberry Pi boards:');
...
if (require.main === module) {
  const fetcher = new FritzingRaspberryPiFetcher();
  fetcher.run().catch(console.error);
}

module.exports = FritzingRaspberryPiFetcher;
